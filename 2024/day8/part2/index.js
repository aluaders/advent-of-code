        ...
        for(const [key, value] of Object.entries(map)) {
            ...
            for(let i = 0; i < nodes.length - 1; i++) {
                for(let j = i + 1; j < nodes.length; j++) {
                    ...
                    let iteration = 0;
                    let x = nodes[i].x - iteration * distance.x;
                    let y = nodes[i].y - iteration * distance.y;

                    while(x >= 0 && y >=0 && x < limits.x && y < limits.y) {
                        if(antinodes.filter(node => node.x === x && node.y === y).length === 0) {
                            antinodes.push({x,y});
                        }

                        iteration++;
                        x = nodes[i].x - iteration * distance.x;
                        y = nodes[i].y - iteration * distance.y;
                    }
